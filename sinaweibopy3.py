...
class APIClient(object):
    ...
    # Custom function:Used to get the relationships
    def get_friendships(self, s_id, t_id):
        result = _http_get('%s' % (self.api_url) + 'friendships/show.json',
    ...
    def get_user_info(self, s_id):
        result = _http_get('%s' % (self.api_url) + 'friendships/show.json',
                           access_token=self.access_token,
                           source_id=s_id,
                           target_id='1969412873',
                           )
        return result['source']['screen_name']
